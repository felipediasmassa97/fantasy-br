...
-- For each as_of_round, get the last 10 rounds up to that point
round_windows as (
    select
        r1.as_of_round_id,
        r2.round_id,
        row_number() over (partition by r1.as_of_round_id order by r2.round_id desc) as round_rank
    from all_rounds r1
    cross join (select distinct round_id from {{ ref('int_players') }} where season = 2026) r2
    where r2.round_id <= r1.as_of_round_id
),

last_10_per_round as (
...
player_rounds as (
    select
        ...
        p.round_id,
        p.has_played,
        row_number() over (partition by lr.as_of_round_id, p.id order by p.round_id desc) as round_rank
    from {{ ref('int_players') }} p
    inner join last_10_per_round lr on p.round_id = lr.round_id
    where p.season = 2026
),
...
-- Get last 10 played matches per player up to each as_of_round
last_10_played as (
    select
        r.as_of_round_id,
        p.id,
