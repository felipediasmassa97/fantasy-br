...
-- Get home rounds per player up to each as_of_round
player_home_rounds as (
    ...
    where p.season = 2026 and p.is_home = true and p.round_id <= r.as_of_round_id
),
...
-- Get last 5 home played matches per player up to each as_of_round
last_5_home_played as (
    ...
    from {{ ref('int_players') }} p
    cross join all_rounds r
    where p.season = 2026 and p.is_home = true and p.has_played = true and p.round_id <= r.as_of_round_id
),
