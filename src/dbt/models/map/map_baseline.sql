...
Combines long-term ability (last season) with recent performance (this season)
to estimate a player's baseline expected points.
...
select
    ...
    position_pts_avg,
    -- Baseline ability calculation
    case
        when has_last_season_data then
            -- Player with sufficient last season data: 60% last season + 40% this season
            0.6 * pts_avg_last_season + 0.4 * coalesce(pts_avg_this_season, 0)
        else
            -- Rookie or new arrival: 70% this season + 30% position average
